...
class ControllerCatalogFilter extends Controller {
	...
	public function sendFilters() {	
		$json = array();	
		$this->load->model('catalog/filter');
		$category_list = preg_split ('/,/' , $this->request->get['category_value']);
		//$category_list = explode(',', $this->request->get['category_value']);
		//print_r($category_list);
		for($i = 0; $i< sizeof($category_list) ; $i++){
			$filters = $this->model_catalog_filter->requestFilters($category_list[$i]); 
		...
		//foreach ($category_list as $category_value) {
		//$filters += $this->model_catalog_filter->requestFilters($category_value); 
		//}
