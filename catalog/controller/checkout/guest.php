...
class ControllerCheckoutGuest extends Controller {
	public function index() {
		...
		if (isset($this->session->data['guest']['otp'])) {
			$data['otp'] = $this->session->data['guest']['otp'];
		} else {
			$data['otp'] = '';
		}
	...
	public function save() {
		...
		if (!$json) {
			...
			if ((utf8_strlen($this->request->post['telephone']) < 3) || (utf8_strlen($this->request->post['telephone']) > 14)) {
		...
		if (!$json) {
			...
			$this->session->data['guest']['otp'] = $this->request->post['input-otp'];
			
			$this->load->model('account/otpvalidation');

			$otp_info = $this->model_account_otpvalidation->validate($this->request->post['input_otp']);
